...
namespace StudioCore.ParamEditor
{
    ...
    public class ParamEditorView
    {
        ...
        public void ParamView(bool doFocus, bool isActiveView)
        {
            ...
            {
                var primary = ParamBank.PrimaryBank.GetVanillaAnyDiff(paramKey);
            ...
            else
            {
                ...
                if (pinnedRowList.Count != 0)
                {
                    foreach (int rowID in pinnedRowList)
                    {
                        ...
                        RowColumnEntry(null, -1, activeParam, null, para[rowID], decorator, ref scrollTo, false, true);
                    }
                ...
                for (int i = 0; i < rows.Count; i++)
                {
                    ...
                    if (enableGrouping)
                    {
                        ...
                        if (prev != null && next != null && prev.ID + 1 != currentRow.ID && currentRow.ID + 1 == next.ID)
                            ImGui.Separator();
                        RowColumnEntry(selectionCache, i, activeParam, rows, currentRow, decorator, ref scrollTo, doFocus, false);
                        if (prev != null && next != null && prev.ID + 1 == currentRow.ID && currentRow.ID + 1 != next.ID)
                            ImGui.Separator();
                    }
                    else
        ...
        private void RowColumnEntry(bool[] selectionCache, int selectionCacheIndex, string activeParam, List<Param.Row> p, Param.Row r, IParamDecorator decorator, ref float scrollTo, bool doFocus, bool isPinned)
        {
