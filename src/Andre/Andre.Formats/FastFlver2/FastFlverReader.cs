...
/*
 * FastFlverHandler must be a struct so that an individual fastflverreader is created by the JIT - methods from FastFlverHandler should be inlined
 */
public sealed class FastFlverReader<FastFlverHandler> where FastFlverHandler : struct, IFastFlverHandler
{
    public bool Load(Memory<byte> bytes, bool isDeS, bool isDaS)
    {
        if (isDeS)
        {
            var FlverDeS = FLVER0.Read(bytes);
            return LoadInternalDeS(FlverDeS, isDeS);
        }
        else if (isDaS)
        {
            var Flver = FLVER2.Read(bytes);
            return LoadInternal(Flver, isDeS);
        }
        else
        {
            ...
            return LoadInternalFast(br, isDeS);
        }
    }

    public bool Load(string path, bool isDeS, bool isDaS)
    {
        ...
        return Load(accessor.Memory, isDeS, isDaS);
    }

    private bool LoadInternalDeS(FLVER0 FlverDeS, bool isDeS)
    {
        var GPUMeshes = new FlverSubmesh[FlverDeS.Meshes.Count()];
        var GPUMaterials = new FlverMaterial[FlverDeS.Materials.Count()];
        var Bones = FlverDeS.Bones;
        var BoneTransforms = new List<Matrix4x4>();
        ...
        for (var i = 0; i < FlverDeS.Materials.Count(); i++)
        {
            ...
            ProcessMaterial(FlverDeS.Materials[i], GPUMaterials[i], isDeS);
        }
        ...
        for (var i = 0; i < FlverDeS.Meshes.Count(); i++)
        {
            ...
            ProcessMesh(ref FlverDeS, ref GPUMaterials, flverMesh, GPUMeshes[i]);
        }
    ...
    private bool LoadInternal(FLVER2 Flver, bool isDeS)
    {
        var GPUMeshes = new FlverSubmesh[Flver.Meshes.Count()];
        var GPUMaterials = new FlverMaterial[Flver.Materials.Count()];
        var Bones = Flver.Bones;

        for (var i = 0; i < Flver.Materials.Count(); i++)
        {
            GPUMaterials[i] = new FlverMaterial();
            ProcessMaterial(Flver.Materials[i], GPUMaterials[i], isDeS);
        }
    ...
    private bool LoadInternalFast(BinaryReaderEx br, bool isDeS)
    {
        // Parse header
        br.BigEndian = false;
        ...
        var GPUMeshes = new FlverSubmesh[meshCount];
        var GPUMaterials = new FlverMaterial[materialCount];
        //Bones = Flver.Bones;

        for (var i = 0; i < materialCount; i++)
        {
            GPUMaterials[i] = new FlverMaterial();
            ProcessMaterial(GPUMaterials[i], isDeS, br, ref materials[i], textures, unicode);
        }
    ...
    private unsafe void ProcessMesh(ref FLVER0 FlverDeS, ref FlverMaterial[] GPUMaterials, FLVER0.Mesh mesh, FlverSubmesh dest)
    {
        dest.Material = GPUMaterials[mesh.MaterialIndex];

        //choice to modify by bone belongs in impl?
        /*
        ...
        //additional call to gen pickingverts and map buffers here

        if (dest.Material.LayoutType == MeshLayoutType.LayoutSky)
        {
            FillVerticesNormalOnly(mesh, dest.Material.VertexSize, mesh.Vertices.Count);
        }
        else if (dest.Material.LayoutType == MeshLayoutType.LayoutUV2)
        {
            FillVerticesUV2(mesh, dest.Material.VertexSize, mesh.Vertices.Count);
        }
        else
        {
            FillVerticesStandard(mesh, dest.Material.VertexSize, mesh.Vertices.Count);
        }
        ...
        {
            //move assignment into out function from impl
            /*
            if (is32bit)
            {
        ...
        //additional call to unmap buffers here
    ...
    private unsafe void ProcessMesh(ref FLVER2 Flver, ref FlverMaterial[] GPUMaterials, FLVER2.Mesh mesh, FlverSubmesh dest)
    {
        dest.Material = GPUMaterials[mesh.MaterialIndex];
    ...
    private unsafe void ProcessMaterial(IFlverMaterial mat, FlverMaterial dest, bool isDeS)
    {
    ...
    private unsafe void ProcessMaterial(FlverMaterial dest, bool isDeS, BinaryReaderEx br,
